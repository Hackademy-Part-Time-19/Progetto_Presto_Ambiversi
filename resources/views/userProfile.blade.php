<x-layout>
    <div class="headerImage">
        ...
        <div style="border-top:#2c2c2c 1px solid;height:auto ">
            ...
            style="background-color:#31ab4e;color: #ebeaea;border-radius: 0px;margin:0px;;font-family: CormorantGaramond; opacity: 1; transition: opacity 1s;"
        ...
        <div class="col-12 col-md-7 col-lg-8 col-ml-8 col-sm-12 p-0">
                ...
                <div class="mb-3">
                    ...
                    <div style="display: flex; justify-content: space-between;">
                        ...
                        <div style="width: 45%;">
                            ...
                            <p style="height: auto;padding:10px 0px; width:100% ;border-bottom:#ababab 0.8px solid;margin:0px;text-align:start;padding-left:10px;font-family:CoromorantGaramond  ">{{ Auth::user()->telephone_number ?? '+39 0471 1655928' }}</p>
            ...
            <div style="padding-bottom: 0px" class="boxContent boxMyArticle">
                ...
                <h5 style="text-align: start;font-family:CoromorantGaramond;border-bottom:#2c2c2c 0.5px solid;width:30%" >Articoli revisionati</h5>
...
@else
<form style="display: flex;width:45px;margin-right:5px" id="favoriteForm" action="{{ route('announcements.addToFavorites', ['announcement' => $announcement->id]) }}" method="POST">
    @csrf
    <button style="width: 45px;" id="toggleHeart" type="submit">
...
<h5  style="text-align: start;font-family:CoromorantGaramond;border-bottom:#2c2c2c 0.5px solid;width:30%" >Articoli da revisionare</h5>
...
<div class="boxContent boxSetInfoPers">
    ...
    <div class="mb-3">
        ...
        <input id="name" name="name" style="height: 50px" value="{{ Auth::user()->name }}"
        type="text" class="form-control @error('name') is-invalid @enderror">
        @error('name')
        <span class="fst-italic text-danger small">{{$message}}</span>
        @enderror

        <label style="text-align: start; width: 100%;margin-top:40px" for="email"
        class="block text-sm font-medium text-gray-700" >Email</label>
        <input id="email" name="email" style="height: 50px"
        value="{{ Auth::user()->email }}" type="email" class="form-control @error('email') is-invalid @enderror">
        @error('email')
        <span class="fst-italic text-danger small">{{$message}}</span>
        @enderror


        <div style="display: flex; justify-content: start;margin:50px">
            <button class="bottoneMostaPasSave" type="submit"> Salva</button>
        </div>

    </form>
    <form action="{{route('number-upload')}}" method="post">
        @csrf

    <div style="width: 45%">
        <label style="text-align: start; width: 100%;margin-top:40px"  for="telephone_number"
        class="block text-sm font-medium text-gray-700">Telefono</label>
        <input id="telephone_number" style="height: 50px" placeholder="{{ Auth::user()->telephone_number ?? '+39 0471 1655928' }}"
        type="text" name="telephone_number" class="form-control">
    </div>
</div>
...
<form action="{{route('photo-upload')}}" method="POST" enctype="multipart/form-data">
    @csrf
<label style="text-align: start; width: 100%;margin-top:40px" for="image"
class="block text-sm font-medium text-gray-700">Cambia immagine</label>

    <input id="image" name="image" style="height: 50px"
    type="file" class="form-control">
    <div style="display: flex; justify-content: start;margin:50px">
        <button class="bottoneMostaPasSave" type="submit" value="upload"> {{Auth::user()->profile_photo_path !==null ? 'Aggiorna' : 'Salva' }}  immagine profilo</button>
    </div>
</form>
...
<div style="display: flex; justify-content: space-between">
    <input id="current_password" name="current_password" style="height: 50px" placeholder="Current password"
...
<div style="display: flex; justify-content: space-between">
    <input id="password" name="password" style="height: 50px" placeholder="Password"
...
<div style="display: flex; justify-content: space-between">
    <input id="password_confirmation" name="password_confirmation" style="height: 50px" placeholder="Password confirmation"
...
<div style="display: flex; justify-content: start;margin:50px">
    <button class="bottoneMostaPasSave" type="submit"> Aggiorna password</button>
</div>
